...
class Vehicles():
    def __init__(self, num_vehicles, mode = 'CP+TR'):
        ...
        # Policy Parameter
        if mode == "CP+TR":
            self.DQN_target = DQN(22, 1 + 57 * 5).to(device)
            self.DQN_training = DQN(22, 1 + 57 * 5).to(device)
        elif mode == "CP":
            self.DQN_target = DQN(22, 1 + 1 * 5).to(device)
            self.DQN_training = DQN(22, 1 + 1 * 5).to(device)
    ...
    def decide(self, exploration_rate=0):
        ...
        decision_table = []
        for s in self.S:
            decision_table.append(vehicle_decide(s, self.DQN_training, exploration_rate, self.mode))
    ...
    def update(self, feedback_table, x_table, new_route_table, new_route_time_table,
        ...
        for i in range(len(feedback_table)):
            ...
            x = x_table[i]
            x_real_dest = self.X_real_dest[i]
            ...
            if feedback is not None:
                if feedback[4][0] is not None:
                    x_real_dest[int(2 * (3 - x[1]-1))] = feedback[4][0]
                    x_real_dest[int(2 * (3 - x[1]-1)) + 1] = feedback[4][1]
            ...
            self.X_real_dest[i] = x_real_dest
            self.X[i] = x
        ...
        for n in range(self.number):
            x = self.X[n]
            x_real_dest = self.X_real_dest[n]
            ...
            if x[11] == 1:
                ...
                if x[1] < 3:  # if with passenger, update its next location
                    ...
                    x, x_real_dest = dropoff(x, x_real_dest)

            self.X[n] = x
            self.X_real_dest[n] = x_real_dest
    ...
    def draw_map(self, current_time, TEST_VEHICLE, time_interval=3):
        if current_time % time_interval == 0:
            ...
            x_real_dest = self.X_real_dest[TEST_VEHICLE]
            ...
            if occupied_seats > 0:
                for i in range(occupied_seats):
                    ...
                    final_points.append(ast.literal_eval(self.zone_lookup.loc[x_real_dest[1+2*i], '(lat,lon)']))
            ...
            map = get_map(route, loc, destination_points, final_points, destination_zones, onboard_remaining_time,
                          detour_time, map)
            map.save("Validation/CP+TR/" + "_" + str(current_time) + ".html")
...
# 2 vehicle decision:
def vehicle_decide(s, DQN_training, exploration_rate, mode ='CP+TR'):
...
def dropoff(x,x_real_dests):
    ...
    temp = np.zeros(11)
    real = np.zeros(len(x_real_dests))
    for i in range(occupied_seats):
        temp[2 + i] = x[2 + passenger[i]]
        temp[5 + i] = x[5 + passenger[i]]
        temp[8 + i] = x[8 + passenger[i]]
        real[0 + 2*i] = x_real_dests[0+2*passenger[i]]
        real[1 + 2*i] = x_real_dests[1+2*passenger[i]]
