...
def training_curve(training_loss_steps, total_reward_eps, contradiction_rate_eps, average_detour_eps, curve_save_path):
    ...
    rolling_avg2 = s2.rolling(window=200).mean()
    rolling_max2 = s2.rolling(window=200).max()
    rolling_min2 = s2.rolling(window=200).min()
    ...
    rolling_avg3 = s3.rolling(window=200).mean()
    ...
    rolling_avg4 = s4.rolling(window=200).mean()
    ...
    plt.savefig(curve_save_path)  # Show the plot 'Training plot/training_0725.png'
    plt.show()

def training_record(training_loss_steps, total_reward_eps, contradiction_rate_eps, average_detour_eps, record_save_path):
