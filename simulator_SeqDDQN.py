...
from Vehicles_SeqDDQN import *
from Central_SeqDDQN import Central
...
# SAVE PATH
Load_Path = 'Save/DQN_CP+TR_SeqDDQNCP_200car.pt'
Load_Path = 'Save/DQN_CP+TR_SeqDDQN_200car.pt'
POLICY_PATH = 'Save/DQN_CP+TR_SeqDDQN'+ MODE + '_200car_TEST.pt'
training_curve_path = 'Training plot/training_SeqDDQN'+ MODE + '_200car_TEST.png'
training_record_path = 'recording/recording_SeqDDQN'+ MODE + '_200car_TEST.pkl'
...
Vehicles = Vehicles(NUM_VEHICLES, mode=MODE)
...
for eps in range(NUM_EPISODES+1):
    ...
    for delta_t in range(TEST_TIME):
        ...
        for n in range(Vehicles.number):
            ...
            if s is not None:
                ...
                if len(Vehicles.X_experience[n]) == 5:
                    ...
                    Vehicles.X_experience[n] = []

            if eps % 50 == 0:
                decision = vehicle_decide(s, Vehicles.DQN_training, exploration_rate=0, mode= MODE)

            else:
                decision = vehicle_decide(s, Vehicles.DQN_training, exploration_rate=exploration_rate, mode=MODE)
        ...
        print("vehicle {0}'s updated information is {1} and real destinations are {2}"
              .format(TEST_VEHICLE, Vehicles.X[TEST_VEHICLE], Vehicles.X_real_dest[TEST_VEHICLE]))
    ...
    if (Central.Pickup + Central.Contradiction) > 0:
        ...
        if eps % 50 != 0:
            ...
            print('Total revenue is:', Central.Total_Revenue)
